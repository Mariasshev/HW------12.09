...
Videocard::Videocard(const Videocard& laptop)
{
	this->video_model = new char[strlen(laptop.video_model) + 1];
	strcpy_s(video_model, strlen(laptop.video_model) + 1, laptop.video_model);

	this->video_price = laptop.video_price;
}
Videocard::~Videocard() {
	delete[] video_model;
}